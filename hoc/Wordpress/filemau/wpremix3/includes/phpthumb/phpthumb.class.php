...
//		}
		...
		switch ($this->thumbnailFormat) {
			case 'wbmp':
				if (!@$builtin_formats['wbmp']) {
					$this->DebugMessage('GD does not have required built support for WBMP output', __FILE__, __LINE__);
					ob_end_clean();
					return false;
				}
			...
			case 'jpg':  // should be "jpeg" not "jpg" but just in case...
				if (!@$builtin_formats['jpg']) {
					$this->DebugMessage('GD does not have required built support for JPEG output', __FILE__, __LINE__);
					ob_end_clean();
					return false;
				}
				ImageJPEG($this->gdimg_output, null, $this->thumbnailQuality);
			...
			case 'png':
				if (!@$builtin_formats['png']) {
					$this->DebugMessage('GD does not have required built support for PNG output', __FILE__, __LINE__);
					ob_end_clean();
					return false;
				}
			...
			case 'gif':
				if (!@$builtin_formats['gif']) {
					$this->DebugMessage('GD does not have required built support for GIF output', __FILE__, __LINE__);
					ob_end_clean();
					return false;
				}
	...
	// private: reitializator (call between rendering multiple images with one object)
	function resetObject() {
		...
		$this->phpThumb(); // reitialize some class variables
		return true;
	}
				...
				if (eregi('jpe?g', $outputFormat) && $this->q) {
					if ($this->ImageMagickSwitchAvailable(array('quality', 'interlace'))) {
						...
						if ($this->config_output_interlace) {
							...
							$commandline .= ' terlace line '; // Use Line or Plane to create an interlaced PNG or GIF or progressive JPEG image
						}
					}
				}
		...
		if ($filename && ($getimagesizeinfo = @GetImageSize($filename))) {
			if (!$this->SourceImageIsTooLarge($getimagesizeinfo[0], $getimagesizeinfo[1])) {
				...
				switch (@$getimagesizeinfo[2]) {
					...
					case 14: // IFF
					case 16: // XBM
						$this->DebugMessage('No built image creation function for image type "'.@$getimagesizeinfo[2].'" ($getimagesizeinfo[2])', __FILE__, __LINE__);
						break;
...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
	...
	function ImageCreateFromStringReplacement(&$RawImageData, $DieOnErrors=false) {
		...
		if (phpthumb_functions::gd_is_bundled()) {
			$this->DebugMessage('ImageCreateFromStringReplacement() calling built ImageCreateFromString()', __FILE__, __LINE__);
