...
class phpthumb_functions {
	...
	function SafeURLread($url, &$error, $timeout=10, $followredirects=true) {
		...
		if (function_exists('curl_version') && !phpthumb_functions::FunctionIsDisabled('curl_exec')) {
			$ch = curl_init();
			curl_setopt($ch, CURLOwpr_URL, $url);
			curl_setopt($ch, CURLOwpr_HEADER, false);
			curl_setopt($ch, CURLOwpr_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOwpr_BINARYTRANSFER, true);
			curl_setopt($ch, CURLOwpr_TIMEOUT, $timeout);
...
if (!function_exists('imagealphablending')) {
	// built function requires PHP v4.0.6+ *and* GD v2.0.1+
...
if (!function_exists('imagesavealpha')) {
	// built function requires PHP v4.3.2+ *and* GD v2.0.1+
